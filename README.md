...
> What actually helps me feel and function better? How do I avoid making the day harder than it needs to be?
...
The point is not perfection, it is to notice patterns early enough to do something useful with them.

Noticing is built around a simple personal rule:

> Support your mind and body first. Then reassess.

---

## Live Demo at:
# noticing-six.vercel.app
...
The demo account contains sample data so the dashboard, logs page, and insights page can be viewed without creating a personal account.
...
Some days feel terrible for some reason, but then you look closer and realize:

- You had no water.
- You ate your first meal at 4pm.
- You never went outside.
- You scrolled instead of reading.
- You had a high-intensity thought loop.
- You tried to fix your entire life while underfed and dehydrated.

Noticing is a way to catch that pattern and reduce avoidable suffering by making the basics visible.
...
Some days just need the minimum effective intervention:
Atleast:
- drink water.
- eat something healthy and well balanced.
- get sunlight.
- move for five minutes.
- take a shower.
- write down the emotion or thought.
- do one useful thing.
  The goal is to keep your body supported enough that life does not become unnecessarily harder.

 The app tells the user what probably needs attention next.
...
This keeps the app easier to navigate as features grow.
...
This is important because `logs.id` is the row ID generated by the database, while `logs.user_id` is the owner ID from authentication.
...
That structure avoids putting repeated data into one log row.
...
Supabase RLS enforces ownership at the database level instead of solely relying on frontend checks.
